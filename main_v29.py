...
if "mqtt_data" not in st.session_state:
    st.session_state.mqtt_data = {
        "stage_map": {0: [], **{i: None for i in range(1, 7)}},  # Only 0 stage can have multiple sessions
        "sessions": {},
        "active_sessions": [],
        "completed_sessions": [],
        "next_session_id": 1,
...
with st.form("start_session_form"):
    ...
    if submitted and st.session_state.free_range_mode:
        ...
        st.session_state.mqtt_data["stage_map"][0].append(session_id)
        st.session_state.mqtt_data["active_sessions"].append(session_id)
        st.session_state.mqtt_data["next_session_id"] += 1
        save_data()
        st.success(f"Session '{session_name}' started in Stage 0.")
...
stage_themes = {
    0: {"name": "Free Range", "color": "#A9A9A9"},
...
if st.session_state.mqtt_data["active_sessions"]:
    with st.form("progress_form"):
        ...
        deleted = st.form_submit_button("Delete Session")
        if deleted and session_to_progress:
            if (st.session_state.mqtt_data["sessions"][session_to_progress]["current_stage"]):
            ...
            save_data()
            st.success(f"Session '{session_to_progress}' has been deleted and marked as completed.")
