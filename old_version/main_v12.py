    ...
    with col:
        ...
        with stage_container:
            ...
            if session_id:
                ...
                # Build the session card HTML
                session_card_html = f"""
                <div style='border: 1px solid #888; padding: 10px; border-radius: 5px;'>
                    <h4 style='margin-bottom: 5px;'>{session_name}</h4>
                    <p style='margin-top: 0px; color: #555;'>{session_id}</p>
                    <hr style='margin: 10px 0;'>
                    <div style='display: flex; justify-content: space-between; align-items: center;'>
                        <form action="/" method="post">
                            <button name="decrement" value="{session_id}" style="font-size: 24px;">➖</button>
                        </form>
                        <p style='margin: 0; font-size:24px;'>{session_score}</p>
                        <form action="/" method="post">
                            <button name="increment" value="{session_id}" style="font-size: 24px;'>➕</button>
                        </form>
                    </div>
                </div>
                """
                # Render the session card
                st.markdown(session_card_html, unsafe_allow_html=True)
                # Handle button clicks
                if f"inc_{session_id}" not in st.session_state:
                    st.session_state[f"inc_{session_id}"] = False
                if f"dec_{session_id}" not in st.session_state:
                    st.session_state[f"dec_{session_id}"] = False
