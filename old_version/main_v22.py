...
def on_message(client, userdata, message):
    ...
    print(f"Added message to queue. Queue size: {message_queue.qsize()}")
...
def process_message_queue():
    print(f"Queue size before processing: {message_queue.qsize()}")
    while not message_queue.empty():
        ...
        for stage, topics in STAGE_TOPICS.items():
            print(f"topic: {topic}, topics: [{topics}], stage: {stage}")
            if topic in topics:  # Match topic to stage
                with data_lock:
                    session_id = mqtt_data["stage_map"].get(stage)  # Get session in the current stage
                    if session_id:
                        # Check if the topic has a scoring entry in topic_values
                        if topic in topic_values:
                            ...
                            print(f"No scoring configuration for topic '{topic}'.")
                    else:
                        print(f"No session in stage {stage} for topic '{topic}', message ignored.")
        save_data()
        sync_data_to_session_state()
...
# # Process messages from the MQTT thread
