...
for i, col in enumerate(cols, start=1):
    ...
    with col:
        ...
        with stage_container:
            ...
            #     <defs>
            #         <linearGradient id="gate_gradient_{i}" x1="0%" y1="0%" x2="100%" y2="0%">
            #             <stop offset="0%" style="stop-color:{'#4CAF50' if session_id is None else '#F44336'}; stop-opacity:1" />
            #             <stop offset="100%" style="stop-color:{'#81C784' if session_id is None else '#E57373'}; stop-opacity:0.5" />
            #         </linearGradient>
            #     </defs>
            #     <rect x="0" y="10" width="200" height="30"
            #         fill="url(#gate_gradient_{i})"
            #         stroke="{'#2E7D32' if session_id is None else '#D32F2F'}"
            #         stroke-width="2"
            #         rx="10" ry="10"/>
            #     <text x="100" y="30" text-anchor="middle" fill="white" font-weight="bold">
            #         {f"OPEN" if session_id is None else "CLOSED"}
            #     </text>
            # </svg>
            # """
            # st.markdown(gate_svg, unsafe_allow_html=True)
