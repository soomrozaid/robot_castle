...
# This will store messages received by on_message until processed in main thread
pending_messages = []
...
def process_pending_messages():
    # This function is called in main thread after autorefresh
    with data_lock:
        local_messages = pending_messages[:]
        ...
        if topic in topics:
            with data_lock:
                ...
                if session_id and topic in topic_values:
                    ...
                    print(f"Current score for {session_id} before: {mqtt_data['sessions'][session_id]['score']}")
                    if payload == "positive":
                    ...
                    if scored:
                        print(f"New score for {session_id}: {mqtt_data['sessions'][session_id]['score']}")
                        save_data()
                else:
            ...
            with session_card:
                ...
                with btn_col1:
                    if st.button("➖", key=f"dec_{session_id}"):
                        with data_lock:
                            ...
                            print(f"Manually decremented score for {session_id}, new score: {mqtt_data['sessions'][session_id]['score']}")
                            save_data()
                            sync_data_to_session_state()
                with score_col:
                ...
                with btn_col2:
                    if st.button("➕", key=f"inc_{session_id}"):
                        with data_lock:
                            mqtt_data["sessions"][session_id]["score"] += 1
                            print(f"Manually incremented score for {session_id}, new score: {mqtt_data['sessions'][session_id]['score']}")
                            save_data()
                            sync_data_to_session_state()
        else:
            st.write("No session")
