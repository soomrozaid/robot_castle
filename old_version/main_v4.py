    ...
    with col:
        ...
        if session_id:
            ...
            if not MQTT_ENABLED and i == 1:
                increment = st.button(f"Increment Score ({session_id})", key=f"inc_{session_id}")
                decrement = st.button(f"Decrement Score ({session_id})", key=f"dec_{session_id}")
                if increment:
...
st_autorefresh = st.experimental_rerun  # For Streamlit versions >=1.12, use st.experimental_rerun()
st_autorefresh()