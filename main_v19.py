...
def on_connect(client, userdata, flags, rc, properties=None):
    if rc == 0:
        ...
        # Subscribe to dynamically loaded topics from topic_values.json
        for topic in dynamic_topics:
            client.subscribe(topic)
            print(f"Successfully subscribed to dynamic topic {topic}")
    else:
...
def on_message(client, userdata, message):
    ...
    #     return
     ...
     # Handle dynamic scoring logic
    for stage, data in progression_rules["stage_progression"].items():
        if data["trigger_topic"] == topic and data["trigger_message"] == payload:
            with data_lock:
                ...
                if session_id:
                    ...
                    if next_stage:
                        ...
                        if stage == "final":
                            mqtt_data["active_sessions"].remove(session_id)
                            mqtt_data["completed_sessions"].append(session_id)
                            print(f"Session '{session_id}' has completed all stages.")
    ...
    with data_lock:
        ...
        if session_id is not None:
            # Use topic-specific values for score adjustment
            if topic in topic_values:
                ...
                if payload == "positive":
                    ...
                    print(f"Score incremented by {increment_value} for session {session_id}")
                elif payload == "negative":
                    decrement_value = topic_score.get("negative", 0)
                    mqtt_data["sessions"][session_id]["score"] += decrement_value
                    print(f"Score decremented by {decrement_value} for session {session_id}")
                else:
                    print(f"Unsupported payload '{payload}' received for topic {topic}.")
        else:
